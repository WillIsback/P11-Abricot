...
export default async function DisplayProjects ({}){
  ...
  if(!ok || !isProjects(data)) return <p>Error retrieve projects data : {message}</p>
    ...
    <div className="mt-15">
      <ul className="flex flex-row gap-x-4 gap-y-16 flex-wrap">
        {data.projects.map(async (p) => {
          ...
          if(!res.ok || !isTasks(res.data))return <p key={p.id}>erreur de recuperations des tâches projet</p>
          return (
            <li key={p.id}>
