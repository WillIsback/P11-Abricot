...
type SessionFailure = { ok: false; status: number; message: string; validationError: z.ZodError };

export async function encrypt(payload: SessionPayload): Promise<SessionSuccess | SessionFailure> {
  const result = SchemaSessionPayload.safeParse(payload)
  if(!result.success){
    return { ok: false, status: 400, message: "Invalid payload", validationError: result.error };
  }

  const expiresAt = result.data.expiresAt ?? new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
