...
/**
 * Wrapper around ComboboxPrimitive.Root that fixes WAVE accessibility error
 * Base UI generates a hidden input with tabindex="-1" that WAVE flags as
 * "missing form label". This wrapper adds an aria-label to that input.
 */
function Combobox({
	children,
	...props
	...
	React.useEffect(() => {
		...
		// Find and fix the hidden validation input generated by Base UI
		const hiddenInput = containerRef.current.querySelector(
			'input[aria-hidden="true"][tabindex="-1"]',
		);
		if (hiddenInput) {
			hiddenInput.setAttribute("aria-label", "hidden validation field");
		}
	});
