...
export const AuthService = {
    register: async (payload: z.infer<typeof Register>, userId: string): Promise<ApiResult<PostResponse>> => {
    ...
    login: async (payload: z.infer<typeof Login>, userId: string): Promise<ApiResult<PostResponse>> => {
      // 1. Vérifier rate limit
      if (!checkRateLimit(userId, 500, 1)) {
        return { ok: false, status: 429, message: "Trop de demandes, patiente 500ms avant de réessayer" };
      }
