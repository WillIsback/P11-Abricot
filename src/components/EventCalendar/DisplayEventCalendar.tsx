...
import EventCalendar, { CalendarEvent } from '@/components/EventCalendar/EventCalendar';
...
const priorityToColor = (priority: TaskType['priority']): CalendarEvent['color'] => {
  const colorMap = {
    LOW: 'green',
    MEDIUM: 'blue',
    HIGH: 'yellow',
...
export default function DisplayEventCalendar( { tasks } : { tasks: TaskType[]}) {
  ...
  return (
    <main className="p-8 bg-gray-50 rounded-[10px]">
      <h1 className="text-center body-xl">
        Calendrier
      </h1>
      <EventCalendar
        events={events}
